...
async def main():
    ...
    def detection_callback(device, advertising_data):
        ...
        if device.name is not None:            
            ...
            elif pbhub is None and device.name.lower() == "Pixoo".lower():
                pixoo = Pixoo(device.address)
    ...
    async def sendPixooPoint(points):
        ...
        if points < 999:
            ...
            digit_image = pixoo.get_pixel_art(f"{points // 1000}k", points % 1000)
        return digit_image
    ...
    while True:
        ...
        if ppp == True:
            ppp = False
            await sendPixooPoint(POINTS)###PB ROUTINE ADD!!!!! line311
            await sendsb(b"000")
